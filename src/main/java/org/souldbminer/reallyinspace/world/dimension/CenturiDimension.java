...
@RisModElements.ModElement.Tag public class CenturiDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:centuri")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:centuri_portal")
	...
	public CenturiDimension (RisModElements instance) {
		super(instance, 85);
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("centuri"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:centuri")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:centuri"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:centuri"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:centuribiome")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new CenturiItem().setRegistryName("centuri"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	public CustomPortalBlock() {
		...
		setRegistryName("centuri_portal");
	}
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = CenturiDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("centuri_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "centuri_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
				...
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(
							Blocks.WHITE_CONCRETE.getDefaultState().getBlock(),
