...
@RisModElements.ModElement.Tag public class NeptuneDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:neptune")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:neptune_portal")
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("neptune"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:neptune")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:neptune"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:neptune"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:neptunebiome")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new NeptuneItem().setRegistryName("neptune"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	public CustomPortalBlock() {
		...
		setRegistryName("neptune_portal");
	}
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = NeptuneDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("neptune_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "neptune_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
				...
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(
							Blocks.BLUE_ICE.getDefaultState().getBlock(),
