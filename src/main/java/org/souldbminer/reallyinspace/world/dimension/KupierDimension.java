...
@RisModElements.ModElement.Tag public class KupierDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:kupier")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:kupier_portal")
	...
	public KupierDimension (RisModElements instance) {
		super(instance, 156);
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("kupier"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:kupier")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:kupier"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:kupier"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:kupierbiome")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new KupierItem().setRegistryName("kupier"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = KupierDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("kupier_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "kupier_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
