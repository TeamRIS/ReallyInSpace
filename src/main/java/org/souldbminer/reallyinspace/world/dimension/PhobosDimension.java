...
@RisModElements.ModElement.Tag public class PhobosDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:phobos")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:phobos_portal")
	...
	public PhobosDimension (RisModElements instance) {
		super(instance, 159);
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("phobos"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:phobos")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:phobos"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:phobos"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:moon_biomes")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new PhobosItem().setRegistryName("phobos"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = PhobosDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("phobos_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "phobos_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
	...
	public ChunkProviderModded(IWorld world, BiomeProvider provider) {
		super(world, provider, new OverworldGenSettings() {
			public BlockState getDefaultBlock() {
				return MartianStoneBlock.block.getDefaultState();
			}
				...
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(
							MartianStoneBlock.block.getDefaultState().getBlock(),
