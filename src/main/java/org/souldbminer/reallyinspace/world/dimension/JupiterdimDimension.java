...
@RisModElements.ModElement.Tag public class JupiterdimDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:jupiterdim")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:jupiterdim_portal")
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("jupiterdim"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:jupiterdim")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:jupiterdim"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:jupiterdim"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:jupiter")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new JupiterdimItem().setRegistryName("jupiterdim"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	public CustomPortalBlock() {
		...
		setRegistryName("jupiterdim_portal");
	}
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = JupiterdimDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("jupiterdim_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "jupiterdim_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
				...
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(
							Blocks.ORANGE_TERRACOTTA.getDefaultState().getBlock(),
