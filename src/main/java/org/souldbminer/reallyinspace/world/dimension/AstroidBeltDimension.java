...
@RisModElements.ModElement.Tag public class AstroidBeltDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:astroid_belt")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:astroid_belt_portal")
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("astroid_belt"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:astroid_belt")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:astroid_belt"), dimension, null, true);
		}

		type = DimensionType.byName(new ResourceLocation("ris:astroid_belt"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:astroids")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new AstroidBeltItem().setRegistryName("astroid_belt"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	public CustomPortalBlock() {
		...
		setRegistryName("astroid_belt_portal");
	}
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = AstroidBeltDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("astroid_belt_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "astroid_belt_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
