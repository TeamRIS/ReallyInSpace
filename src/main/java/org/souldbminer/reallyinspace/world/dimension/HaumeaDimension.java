...
@RisModElements.ModElement.Tag public class HaumeaDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:haumea")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:haumea_portal")
	...
	public HaumeaDimension (RisModElements instance) {
		super(instance, 149);
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("haumea"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:haumea")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:haumea"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:haumea"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:haumeaflat")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new HaumeaItem().setRegistryName("haumea"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = HaumeaDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("haumea_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "haumea_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
	...
	public ChunkProviderModded(IWorld world, BiomeProvider provider) {
		super(world, provider, new OverworldGenSettings() {
			public BlockState getDefaultBlock() {
				return Blocks.STRIPPED_SPRUCE_WOOD.getDefaultState();
			}
				...
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(
							Blocks.STRIPPED_SPRUCE_WOOD.getDefaultState().getBlock(),
