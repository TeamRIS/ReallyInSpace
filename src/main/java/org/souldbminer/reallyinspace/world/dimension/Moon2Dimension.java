...
@RisModElements.ModElement.Tag public class Moon2Dimension extends RisModElements.ModElement{

	@ObjectHolder("ris:moon_2")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:moon_2_portal")
	...
	public Moon2Dimension (RisModElements instance) {
		super(instance, 90);
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("moon_2"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:moon_2")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:moon_2"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:moon_2"));
	}

	@Override public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[] {
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ris:moonbiome")),
		};
	}

		@Override public void initElements() {
			elements.blocks.add(() -> new CustomPortalBlock());
			elements.items.add(() -> new Moon2Item().setRegistryName("moon_2"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = Moon2Dimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("moon_2_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "moon_2_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
				...
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(
							MoonStoneBlock.block.getDefaultState().getBlock(),
