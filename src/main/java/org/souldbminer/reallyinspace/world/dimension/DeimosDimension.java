...
@RisModElements.ModElement.Tag public class DeimosDimension extends RisModElements.ModElement{

	@ObjectHolder("ris:deimos")
	public static final ModDimension dimension = null;

	@ObjectHolder("ris:deimos_portal")
	...
	public DeimosDimension (RisModElements instance) {
		super(instance, 160);
	...
	@SubscribeEvent public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("deimos"));
	}

	@SubscribeEvent public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("ris:deimos")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("ris:deimos"), dimension, null, false);
		}

		type = DimensionType.byName(new ResourceLocation("ris:deimos"));
	}
	...
	@Override public void init(FMLCommonSetupEvent event) {
		...
		@Override public void initElements() {
			...
			elements.items.add(() -> new DeimosItem().setRegistryName("deimos"));
		}
...
public static class CustomPortalBlock extends NetherPortalBlock {
	...
	private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
		BlockPattern.PatternHelper bph = DeimosDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
...
public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("deimos_portal", Vec3i::compareTo, 300);
...
@SubscribeEvent public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
	try {
		...
		poi = (PointOfInterestType) method.invoke(null, "deimos_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1);
				...
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(
							Blocks.STONE.getDefaultState().getBlock(),
