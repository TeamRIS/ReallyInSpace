...
public class AilenEntity extends RisModElements.ModElement {
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("ris:marsflat").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("ris:astroids").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("ris:moonbiome").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		event.getSpawns().getSpawner(EntityClassification.CREATURE).add(new MobSpawnInfo.Spawners(entity, 31, 4, 4));
	}
