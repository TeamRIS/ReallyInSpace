...
@RisModElements.ModElement.Tag public class RcItem extends RisModElements.ModElement{
	...
	@SubscribeEvent @OnlyIn(Dist.CLIENT) public void onItemDropped(ItemTossEvent event) {
		if(event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof PannelGui.GuiWindow) {
			...
			if(entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					@Override public ITextComponent getDisplayName() {
						return new StringTextComponent("Rocket Controller");
					}
